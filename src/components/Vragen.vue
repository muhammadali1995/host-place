<template>
    <div class="px-4 lg:px-32 mt-6 lg:mt-48">
        <div class="bg-primary flex md:flex-row flex-col">
            <div class="md:flex-1 pl-16 flex items-start justify-center flex-col py-8 md:py-0">
                <p class="text-3xl font-bold tracking-tight text-center text-gray-350 sm:text-4xl">
                    {{ $t('Meer Vragen') }}
                </p>
                <button type="button"
                    class="px-5 mt-5 py-2 text-sm bg-gray-450 text-gray-250 hover:bg-gray-650">{{
                        $t('Neem contact op') }}
                </button>
            </div>
            <img src="./../assets/image.png" alt="image" class="md:flex-1 md:bg-cover md:max-h-96">
        </div>
    </div>
</template>
<script></script>